...
# ======== LOGGING / UNBUFFERED ========
# Чтобы Render не проглатывал логи
try:
...
def _require_env(name: str, default: str | None = None) -> str:
    ...
    if val is None or val == "":
        logging.error("ENV %s is missing/empty", name)
    return val

BOT_TOKEN = _require_env("BOT_TOKEN")
if not BOT_TOKEN:
    logging.critical("BOT_TOKEN is not set. Fix it in Render → Environment.")
    sys.exit(1)

_GROUP_ID_RAW = _require_env("GROUP_ID", "0")
try:
    GROUP_ID = int(_GROUP_ID_RAW)
except Exception:
    logging.critical("GROUP_ID must be int, got %r", _GROUP_ID_RAW)
    sys.exit(1)

ADMIN_IDS = {int(x) for x in _require_env("ADMIN_IDS", "").split(",") if x.strip().isdigit()}

ROLE_TOPICS = {
    "translator": int(_require_env("THREAD_TRANSLATOR_ID", "0")),
    "editor":     int(_require_env("THREAD_EDITOR_ID", "0")),
    "cleaner":    int(_require_env("THREAD_CLEAN_ID", "0")),
    "typesetter": int(_require_env("THREAD_TYPES_ID", "0")),
    "gluer":      int(_require_env("THREAD_GLUE_ID", "0")),
    "curator":    int(_require_env("THREAD_CURATOR_ID", "0")),
    "beta":       int(_require_env("THREAD_BETA_ID", "0")),
    "typecheck":  int(_require_env("THREAD_TYPECHECK_ID", "0")),
}
...
TEST_DEADLINE_DAYS = int(_require_env("TEST_DEADLINE_DAYS", "3"))
PORT = int(_require_env("PORT", "10000"))
...
# Бан-лист
BANNED_IDS = {int(x) for x in _require_env("BANNED_IDS", "").split(",") if x.strip().isdigit()}
...
async def schedule_deadline_notify(user_id: int, role_key: str, started_at: datetime):
    ...
    try:
        ...
        logging.warning("Error posting assignment: %s", e)
    ...
    async with lock:
        ...
        if msg_id:
            try:
                ...
                logging.info("Edit failed, fallback to send: %s", e)
                st["msg_id"] = None
    ...
    if unsupported:
        for msg in unsupported:
            try:
                ...
                logging.warning("Unsupported media forward error: %s", e)
...
# ---- УНИВЕРСАЛЬНАЯ отправка бандла админа кандидату ----
...
async def _send_admin_bundle_to_user(user_id: int, text: str, items: list[Message]):
    ...
    try:
        ...
        logging.exception("admin bundle failed: %s", e)
        return False
...
# ---- /pm для админов (со склейкой исходящих) ----
...
# ---- Свайп-ответ админов (со склейкой исходящих) ----
...
def start_http():
    ...
    logging.info("HTTP server on %s", PORT)
    srv.serve_forever()

# ============ MAIN ============

async def main():
    logging.info("Boot: deleting webhook")
    try:
    ...
    try:
        me = await bot.get_me()
        logging.info("Running bot: @%s (id %s)", getattr(me, "username", "?"), getattr(me, "id", "?"))
    except Exception:
    ...
    try:
        ...
        logging.exception("setup_commands failed")
    ...
    try:
        ...
        logging.exception("start_polling crashed")
        raise
...
if __name__ == "__main__":
    try:
        ...
        logging.info("Shutdown by KeyboardInterrupt")
    except SystemExit as e:
        logging.critical("SystemExit: %s", e)
        raise
    except Exception:
